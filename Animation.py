...
global PlayerName
InputWindow = Tk()
InputWindow.title("INPUT NAME")
InputWindow.geometry("200x200")
InputLabel = Label(InputWindow , text="Plase Enter Your Name:" , fg="purple")
InputLabel.pack()
entry = Entry(InputWindow , bd=4 , fg="dark blue" , cursor="cross")
entry.pack()
def GetName():
	global PlayerName
	global IsResume
	...
	InputWindow.destroy()
InputButtom1 = Button(InputWindow , text="CONFIRM" , width=15 , height = 2 , command=GetName , bg="yellow" , fg="red")
InputButtom1.pack()
InputWindow.mainloop()
UserName = str(PlayerName)
...
def MoveSnake():
	...
	if Positions[0][0] < 0 or Positions[0][2] > Width or Positions[0][3] > Height or Positions[0][1] < 0:
		GameOver = True
		canvas.create_text(Width/2 , Height/2 , fill="red" , font="Arial" , text="GAME OVER\nScore:" + str(Score))
	...
	for i in range(1,len(Snake)):
		if OverLapping(SnakeHeadPosition , canvas.coords(Snake[i])) and IsPause == False and IsBoss == False and IsCheat == False:
			GameOver = True
			canvas.create_text(Width/2 , Height/2 , fill="blue" , font="Arial" , text="GAME OVER")
			MyFile.write(UserName + ": " + str(Score) + "\n")
			MyFile.close()
	if GameOver == False:
...
def GrowSnake():
	...
	if direction == "left" or direction=="moreleft":
		canvas.coords(Snake[LastElement+1] , LastElementPosition[0] + SnakeSize , LastElementPosition[1] , LastElementPosition[2] + SnakeSize , LastElementPosition[3])
	elif direction == "right" or direction=="moreright":
		canvas.coords(Snake[LastElement+1],LastElementPosition[0] - SnakeSize,LastElementPosition[1] , LastElementPosition[2] - SnakeSize , LastElementPosition[3])
	elif direction == "up" or direction=="moreup":
		canvas.coords(Snake[LastElement+1] , LastElementPosition[0] , LastElementPosition[1] + SnakeSize , LastElementPosition[2] , LastElementPosition[3] + SnakeSize)
...
if IsResume == False:
	...
	SnakeSize = 15
	Score = 0
Snake.append(canvas.create_oval(SnakeSize , SnakeSize , SnakeSize*2 , SnakeSize*2 , fill="white"))

Text = "Score: " + str(Score)
ScoreText = canvas.create_text(Width/2, 10 , fill="brown" , font="Arial" , text=Text)
...
SaveLabel = Label(SaveWindow , text="Save Your Name&Score In A File? If Not, just close the page" , fg="purple")
SaveLabel.pack()
...
def GetFile():
	...
	pickle.dump(UserName,list_file)
	list_file.close()
	SaveWindow.destroy()
SaveButtom1 = Button(SaveWindow , text="CONFIRM" , width=15 , height = 2 , command=GetFile , bg="yellow" , fg="red")
SaveButtom1.pack()
SaveWindow.mainloop()
